...
document.querySelectorAll("[data-carousel]").forEach((section) => {
  ...
  function update(next, announce = true) {
    ...
    if (announce)
      status.textContent = `${section.dataset.carousel === "steps" ? "Schritt" : "Bewertung"} ${index + 1} von ${cards.length}`;
  }
  section
...
requestButton.addEventListener("click", () =>
  showNotice(
    "Dies ist eine Design-Vorschau. Die Rezeptanfrage ist noch nicht mit einem medizinischen Dienst verbunden. Es werden keine medizinischen Daten erfasst oder versendet.",
  ),
);
document.querySelectorAll("[data-social]").forEach((button) => {
  button.addEventListener("click", () =>
    showNotice(
      `Der ${button.dataset.social}-Link ist in dieser Vorschau noch nicht hinterlegt und muss vor der Veröffentlichung ergänzt werden.`,
    ),
  );
});
notice
  .querySelectorAll(".dialog-close, .dialog-confirm")
  .forEach((button) => button.addEventListener("click", () => notice.close()));
